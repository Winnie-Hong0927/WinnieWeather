package com.hongwenli.winnieweather.utils;
...
import com.hongwenli.winnieweather.databinding.DialogAddCityBinding;
import com.hongwenli.winnieweather.repository.SearchCityRepository;
import com.hongwenli.winnieweather.ui.adapter.RecommendCityAdapter;
import com.hongwenli.winnieweather.ui.adapter.SearchCityAdapter;
...
public class AddCityDialog {
    ...
    private static final List<LocationBean> beans = new ArrayList<>();
    ...
    public static void show(Context context, List<String> cities, SelectedCityCallback selectedCityCallback) {
        ...
        binding.etSearchCity.setOnEditorActionListener((v, actionId, event) -> {
            if (actionId == EditorInfo.IME_ACTION_SEARCH) {
                ...
                //搜索城市
                //就是输入法的监听，通过XML中EditText的设置，输入法的回车按钮会变成搜索按钮，
                // 这里我们就是对搜索按钮进行点击监听，如果输入了内容，就搜索城市，
                // 通过将关键字传递到适配器中，适配器会在渲染数据的时候根据这个去设置文字的颜色。
                // 点击搜索按钮时，关闭输入法。
                SearchCityRepository.getInstance().searchCity(searchCityResponseMutableLiveData, failed, cityName,false);
                //关闭输入法
